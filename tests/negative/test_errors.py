...
def test_draft_code_missing_file(mock_llm):
    ...
    result = draft_code(
        path="C:/non_existent_file_12345.py",
        instruction="update",
        model="gemini"
    )
...
def test_find_and_draft_edit_malformed_json(tmp_path, mock_llm):
    ...
    mock_llm.return_value = "I cannot find the file, but here is a guess: { 'file': 'main.py' }" # Invalid JSON (single quotes)
    
    with patch("mcp_ai_worker.server.generate_repo_map", return_value="main.py: hello"), \
...
def test_execute_command_timeout(tmp_path, mock_llm):
    ...
    result = execute_command(
        command=f'"{python_exe}" {sleep_script}', 
